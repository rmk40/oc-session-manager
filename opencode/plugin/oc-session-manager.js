...
function broadcastUdpMessage(message) {
  ...
  for (const host of HOSTS) {
    socket.send(buffer, 0, buffer.length, PORT, host, (err) => {
      if (err && DEBUG) {
        console.error(
          `[oc-session-manager] Failed to send to ${host}:${PORT}:`,
          err.message,
        );
      }
    });
...
export const OcSessionManager = async ({ project, directory, client }) => {
  ...
  console.error(
    `[oc-session-manager] Starting plugin for ${dirName} (PID: ${process.pid})`,
  );
  console.error(
    `[oc-session-manager] Broadcasting to: ${HOSTS.join(", ")}:${PORT}`,
  );

  // Discover server URL lazily via API response
  // The SDK client's internal baseUrl isn't always accessible, so we extract it from an actual response
  let serverUrl = null;
  let serverUrlDiscovered = false;

  async function discoverServerUrl() {
    if (serverUrlDiscovered) return serverUrl;
    try {
      ...
      if (resp.response?.url) {
        ...
        console.error(
          `[oc-session-manager] Discovered server URL: ${serverUrl}`,
        );
      }
    } catch (err) {
      debug(`Failed to discover server URL: ${err.message}`);
    }
    serverUrlDiscovered = true;
    return serverUrl;
  }

  // Start discovery in background (don't block startup)
  setTimeout(discoverServerUrl, 100);
  ...
  async function sendStatus(
    ...
    // Ensure server URL is discovered before first status send
    if (!serverUrlDiscovered) {
      ...
      if (event.type === "session.created" && eventSessionID) {
        ...
        try {
          ...
          if (
            ...
            session.data.directory === directory
          ) {
