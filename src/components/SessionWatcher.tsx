...
//
// This component manages the connection to OpenCode servers when viewing a session.
...
import { getConnectionManager } from "../connections.js";
...
export const SessionWatcher = React.memo((): null => {
  ...
  useEffect(() => {
    if (!sessionViewActive || !sessionViewInstance || !sessionViewSessionID)
      return;
    ...
    async function connect() {
      ...
      const serverUrl =
        sessionViewInstance?.serverUrl || "http://127.0.0.1:4096";

      // Check if ConnectionManager already has this server connected
      const connMgr = getConnectionManager();
      const existingServer = connMgr.getServer(serverUrl);
      if (existingServer?.status === "disconnected") {
        actions.setSessionViewError(
      ...
      try {
        // Use existing client from ConnectionManager if available, otherwise create new
        const client = existingServer?.client || getOpencodeClient(serverUrl);
        ...
        for await (const event of events.stream) {
          ...
          if (eventSessionID && eventSessionID !== sessionViewSessionID)
            continue;
    ...
    async function buildTree(
      ...
      try {
        ...
        const sorted = (children.data || []).sort(
      ...
      if (part.type === "text") {
        ...
        for (const line of text.split("\n")) {
          ...
          for (const w of wrapped)
            lines.push({ type: "text", plain: `│ ${w}`, text: `│ ${w}` });
        }
        ...
        lines.push({
          type: "tool-start",
          plain: `│ ┌─ ${icon} ${state.title || name}`,
          text: `│ ┌─ ${icon} ${state.title || name}`,
        });
