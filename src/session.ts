...
export async function enterSessionView(instance: Instance): Promise<void> {
  ...
  try {
    ...
    const idx = sessionViewSessions.findIndex((s: any) => s.id === instance.sessionID)
    if (idx >= 0) {
...
export async function buildSessionTree(rootSessionID: string): Promise<void> {
  ...
  async function addSessionAndChildren(sessionID: string, depth: number): Promise<void> {
    try {
      const sessionResp = await sessionViewClient.session.get({ path: { id: sessionID } })
      ...
      try {
        ...
        children.sort((a: any, b: any) => (a.time?.created || 0) - (b.time?.created || 0))
...
export async function switchSession(direction: 'next' | 'prev'): Promise<void> {
  ...
  if (direction === 'next') {
    ...
    newIndex = (sessionViewSessionIndex - 1 + sessionViewSessions.length) % sessionViewSessions.length
  }
...
export function renderSessionViewLines(): void {
  ...
  for (const msg of sessionViewMessages) {
    ...
    const costInfo = info.cost ? ` ${ANSI.dim}$${info.cost.toFixed(4)}${ANSI.reset}` : ''
...
function renderToolPart(part: MessagePart, lines: RenderedLine[]): void {
  ...
  if (status === 'completed' && output) {
    ...
    const displayLines = truncated ? outputLines.slice(0, maxLines) : outputLines
...
export function scrollSessionView(
  ...
  const maxScroll = Math.max(0, sessionViewRenderedLines.length - contentHeight)
