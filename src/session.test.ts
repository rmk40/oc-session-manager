...
const { mockState, mockClient, mockRender, mockSessionGet, mockSessionChildren,
  mockSessionMessages, mockSessionAbort, mockSessionPrompt, mockEventSubscribe,
  mockPostPermission } = vi.hoisted(() => {
...
vi.mock('./state.js', () => ({
  ...
  get sessionViewClient() { return mockState.sessionViewClient },
  ...
  get sessionViewMessages() { return mockState.sessionViewMessages },
  ...
  get sessionViewEventAbort() { return mockState.sessionViewEventAbort },
  get termWidth() { return mockState.termWidth },
  get termHeight() { return mockState.termHeight },
  setSessionViewActive: vi.fn((val: boolean) => { mockState.sessionViewActive = val }),
  setSessionViewClient: vi.fn((val: any) => { mockState.sessionViewClient = val }),
  setSessionViewInstance: vi.fn((val: Instance | null) => { mockState.sessionViewInstance = val }),
  setSessionViewSessionID: vi.fn((val: string | null) => { mockState.sessionViewSessionID = val }),
  setSessionViewMessages: vi.fn((val: Message[]) => { mockState.sessionViewMessages = val }),
  setSessionViewScrollOffset: vi.fn((val: number) => { mockState.sessionViewScrollOffset = val }),
  setSessionViewRenderedLines: vi.fn((val: RenderedLine[]) => { mockState.sessionViewRenderedLines = val }),
  setSessionViewInputMode: vi.fn((val: boolean) => { mockState.sessionViewInputMode = val }),
  setSessionViewInputBuffer: vi.fn((val: string) => { mockState.sessionViewInputBuffer = val }),
  setSessionViewConfirmAbort: vi.fn((val: boolean) => { mockState.sessionViewConfirmAbort = val }),
  setSessionViewError: vi.fn((val: string | null) => { mockState.sessionViewError = val }),
  setSessionViewConnecting: vi.fn((val: boolean) => { mockState.sessionViewConnecting = val }),
  setSessionViewStatus: vi.fn((val: string) => { mockState.sessionViewStatus = val }),
  setSessionViewSessions: vi.fn((val: any[]) => { mockState.sessionViewSessions = val }),
  setSessionViewSessionIndex: vi.fn((val: number) => { mockState.sessionViewSessionIndex = val }),
  setSessionViewSessionTitle: vi.fn((val: string) => { mockState.sessionViewSessionTitle = val }),
  setSessionViewEventAbort: vi.fn((val: AbortController | null) => { mockState.sessionViewEventAbort = val }),
...
// Mock server.js
vi.mock('./server.js', () => ({
  isSessionViewerAvailable: vi.fn(() => mockState.sdkAvailable),
...
import { isSessionViewerAvailable, getOpencodeClient } from './server.js'
...
function createMessage(role: 'user' | 'assistant', parts: MessagePart[] = []): Message {
  return {
    info: { role },
    parts,
  }
...
describe('session', () => {
  ...
  describe('enterSessionView', () => {
    ...
    it('uses localhost fallback when serverUrl is not set', async () => {
      ...
      mockSessionGet.mockResolvedValue({ data: { id: 'session-123', status: 'idle' } })
    ...
    it('fetches session info and updates state', async () => {
      ...
      mockSessionGet.mockResolvedValue({
        data: { id: 'session-123', status: 'running', title: 'Working on feature' },
      })
      mockSessionChildren.mockResolvedValue({ data: [] })
      mockSessionMessages.mockResolvedValue({ data: [] })
      ...
      expect(setSessionViewSessionTitle).toHaveBeenCalledWith('Working on feature')
    })
    ...
    it('follows parentID to find root session', async () => {
      ...
      mockSessionGet
        .mockResolvedValueOnce({
          data: { id: 'child-session', status: 'running', parentID: 'root-session' },
        })
      ...
      expect(mockSessionGet).toHaveBeenCalledWith({ path: { id: 'child-session' } })
      expect(mockSessionGet).toHaveBeenCalledWith({ path: { id: 'root-session' } })
    })
    ...
    it('handles parent session fetch error gracefully', async () => {
      ...
      mockSessionGet
        ...
        .mockResolvedValue({ data: { id: 'child-session', status: 'running' } })
      ...
      expect(setSessionViewError).not.toHaveBeenCalledWith(expect.stringContaining('Parent'))
    })
    ...
    it('handles connection error', async () => {
      ...
      expect(setSessionViewError).toHaveBeenCalledWith('Failed to connect: Connection refused')
  ...
  describe('buildSessionTree', () => {
    ...
    it('builds tree with children', async () => {
      ...
      mockSessionGet
        ...
        .mockResolvedValueOnce({
          data: { id: 'child-1', title: 'Child 1', status: 'busy', parentID: 'root' },
        })
        .mockResolvedValueOnce({
          data: { id: 'child-2', title: 'Child 2', status: 'idle', parentID: 'root' },
        })
      ...
      expect(setSessionViewSessions).toHaveBeenCalledWith([
        { id: 'root', title: 'Root', status: 'idle', parentID: undefined, depth: 0 },
        { id: 'child-1', title: 'Child 1', status: 'busy', parentID: 'root', depth: 1 },
        { id: 'child-2', title: 'Child 2', status: 'idle', parentID: 'root', depth: 1 },
      ])
    })
    ...
    it('handles children fetch error gracefully', async () => {
      ...
      mockSessionChildren.mockRejectedValue(new Error('Failed to fetch children'))
    ...
    it('sorts children by creation time', async () => {
      ...
      mockSessionGet
        .mockResolvedValueOnce({ data: { id: 'root', title: 'Root', status: 'idle' } })
        // After sorting, child-earlier will be fetched first
        .mockResolvedValueOnce({ data: { id: 'child-earlier', title: 'Earlier', status: 'idle', parentID: 'root' } })
        .mockResolvedValueOnce({ data: { id: 'child-later', title: 'Later', status: 'idle', parentID: 'root' } })
  ...
  describe('loadCurrentSessionMessages', () => {
    ...
    it('handles messages fetch error', async () => {
      ...
      expect(setSessionViewError).toHaveBeenCalledWith('Failed to load messages: Failed to load')
    })
  ...
  describe('subscribeToSessionEvents', () => {
    ...
    it('creates abort controller and sets it in state', async () => {
      ...
      expect(setSessionViewEventAbort).toHaveBeenCalledWith(expect.any(AbortController))
    })
    ...
    it('handles event stream error', async () => {
      ...
      expect(setSessionViewError).toHaveBeenCalledWith('Event stream error: Stream failed')
    })
    ...
    it('handles session.status event', async () => {
      ...
      const events = [
        { type: 'session.status', properties: { sessionID: 'session-123', status: 'running' } },
      ]
    ...
    it('handles permission.replied event', async () => {
      ...
      const events = [
        { type: 'permission.replied', properties: { sessionID: 'session-123', id: 'perm-1' } },
      ]
      ...
      const mockStream = {
        async *[Symbol.asyncIterator]() {
          while (true) {
            ...
            yield { type: 'session.idle', properties: { sessionID: 'session-123' } }
          }
        },
      }
      mockEventSubscribe.mockResolvedValue({ stream: mockStream })
    ...
    it('handles tool.execute.before event (no-op)', async () => {
      ...
      const events = [
        { type: 'tool.execute.before', properties: { sessionID: 'session-123' } },
      ]
    ...
    it('handles tool.execute.after event (no-op)', async () => {
      ...
      const events = [
        { type: 'tool.execute.after', properties: { sessionID: 'session-123' } },
      ]
  ...
  describe('refreshMessages', () => {
    ...
    it('fetches and sets messages', async () => {
      ...
      const messages = [createMessage('user', [{ type: 'text', text: 'Hello' }])]
      mockSessionMessages.mockResolvedValue({ data: messages })

      await refreshMessages()

      expect(mockSessionMessages).toHaveBeenCalledWith({ path: { id: 'session-123' } })
      expect(setSessionViewMessages).toHaveBeenCalledWith(messages)
    })
  ...
  describe('abortSession', () => {
    ...
    it('calls session.abort with correct session ID', async () => {
      ...
      expect(mockSessionAbort).toHaveBeenCalledWith({ path: { id: 'session-123' } })
    })
    ...
    it('handles abort error', async () => {
      ...
      expect(setSessionViewError).toHaveBeenCalledWith('Abort failed: Abort failed')
      expect(setSessionViewConfirmAbort).toHaveBeenCalledWith(false)
    })
  ...
  describe('abortInstanceSession', () => {
    ...
    it('calls abort with correct session ID', async () => {
      ...
      expect(mockSessionAbort).toHaveBeenCalledWith({ path: { id: 'session-to-abort' } })
    })
  ...
  describe('respondToPermission', () => {
    ...
    it('removes permission from pending after success', async () => {
      mockState.sessionViewClient = mockClient
      mockState.sessionViewSessionID = 'session-123'
      sessionViewPendingPermissions.set('perm-1', { id: 'perm-1', tool: 'bash' })
  ...
  describe('sendMessage', () => {
    ...
    it('handles send error', async () => {
      ...
      expect(setSessionViewError).toHaveBeenCalledWith('Send failed: Send failed')
    })
  ...
  describe('renderSessionViewLines', () => {
    ...
    it('renders tool parts with status indicators', () => {
      ...
      const lines = vi.mocked(setSessionViewRenderedLines).mock.calls[0][0]
      const toolStart = lines.find((l: RenderedLine) => l.type === 'tool-start')
    ...
    it('renders tool output when completed', () => {
      ...
      const lines = vi.mocked(setSessionViewRenderedLines).mock.calls[0][0]
      const resultLines = lines.filter((l: RenderedLine) => l.type === 'tool-result')
    ...
    it('truncates long tool output', () => {
      ...
      const lines = vi.mocked(setSessionViewRenderedLines).mock.calls[0][0]
      const truncatedLine = lines.find((l: RenderedLine) => l.plain.includes('more lines'))
    ...
    it('renders reasoning parts', () => {
      ...
      const lines = vi.mocked(setSessionViewRenderedLines).mock.calls[0][0]
      const reasoningStart = lines.find((l: RenderedLine) => l.type === 'reasoning-start')
    ...
    it('renders reasoning with text fallback', () => {
      ...
      const lines = vi.mocked(setSessionViewRenderedLines).mock.calls[0][0]
      const reasoningLines = lines.filter((l: RenderedLine) => l.type === 'reasoning')
    ...
    it('skips step-start and step-finish parts', () => {
      ...
      const stepLines = lines.filter(
        (l: RenderedLine) => l.type === 'step-start' || l.type === 'step-finish'
      )
    ...
    it('renders unknown part types with type label', () => {
      ...
      const lines = vi.mocked(setSessionViewRenderedLines).mock.calls[0][0]
      const unknownLine = lines.find((l: RenderedLine) => l.plain.includes('[unknown-type]'))
  ...
  describe('scrollSessionView', () => {
    beforeEach(() => {
      ...
      mockState.sessionViewRenderedLines = Array(50).fill({ type: 'text', text: '', plain: '' })
    ...
    it('handles content smaller than viewport', () => {
      mockState.sessionViewRenderedLines = Array(10).fill({ type: 'text', text: '', plain: '' })
  ...
  describe('edge cases', () => {
    ...
    it('handles tool args formatting', () => {
      ...
      const lines = vi.mocked(setSessionViewRenderedLines).mock.calls[0][0]
      const argsLines = lines.filter((l: RenderedLine) => l.type === 'tool-args')
